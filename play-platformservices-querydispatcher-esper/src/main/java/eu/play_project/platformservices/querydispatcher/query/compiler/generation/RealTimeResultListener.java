...
public class RealTimeResultListener implements UpdateListener{
	...
	public RealTimeResultListener(String[] queries){
		...
		try {
			...
			for(int i = 0; i < newEvents.length; i++){
				...
				for(int j = 0; j < queries.length; j++){
					//TODO ask first, then select variables
					TupleQueryResult result = con.prepareTupleQuery(QueryLanguage.SPARQL, queries[j]).evaluate();
				}
				//TODO
			}
