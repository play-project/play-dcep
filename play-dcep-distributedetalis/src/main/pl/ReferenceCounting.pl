...
incrementReferenceCounter(ID):- ((referenceCounter(ID, ID2, X), (X == -1) -> (X2 is X + 2);  (referenceCounter(ID, ID2, X), X2 is X + 1)), retractall(referenceCounter(ID, ID2, X)), assert(referenceCounter(ID, ID2, X2))).
...
collectGarbage(ID) :- (referenceCounter(ID,_ID2 ,X), X < 1, rdf_retractall(_S,_P,_O,ID), retractall(referenceCounter(ID, _ID2, X)); true).

%Delte unused triples.
%Delete all events older than the last fired event and if they are not in use.
collectGarbage :- (forall(referenceCounter(_ID, ID2, _X), (lastInsertedEvent(Id3), (ID2=<Id3), referenceCounter(_ID, ID2, X), (X < 1)) -> retract(referenceCounter(_D1, ID2, _I2)); (true))).
